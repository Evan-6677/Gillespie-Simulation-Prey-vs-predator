...
N_r = 5          # Number of stochastic runs
T = 30 * 24      # Total simulation time (in hours)
...
# Run multiple simulations and plot each one separately
for i in range(N_r):
    ts, xs = run_forward(x0, T, ground)
    
    plt.plot(ts / 24, xs[:, 0], label='E. coli')
    plt.plot(ts / 24, xs[:, 1], label='Bb')
    plt.plot(ts / 24, xs[:, 2], label='Phage')
    plt.xlabel('Time (days)')
    plt.ylabel('Population')
    plt.title(f'Population Dynamics (Run {i+1})')
    plt.legend()
    plt.show()
